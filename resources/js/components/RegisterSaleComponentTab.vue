<template>
    <div>
        ...
        <FirstRegisterSaleForm v-if = "showFirstRegisterSaleForm == true" @FirstRegisterSaleFormFinishedHandler = "onFirstRegisterSaleFormFinished"></FirstRegisterSaleForm>
...
<script>
    ...
    import FirstRegisterSaleForm from './FirstRegisterSale.vue';
    import Vue from 'vue';
    ...
    export default {
        ...
        data: ()=>{
            ...
            showFirstRegisterSaleForm:false,
            form :{
                age: 18,
                price: 0,
                personalInfo: {},
        ...
        beforeMount(){
            ...
            // this.showQuestionnaire = true;
                ...
                if(urlArr[urlArr.length-1] == 'detailed'){
                    this.showFirstRegisterSaleForm = false;
                    this.showQuestionnaire = false;
                    this.showWizard = true;
                    
                }else{
                    this.showQuestionnaire = false;
                    this.showFirstRegisterSaleForm = true;
                }
        },
        methods: {
            onComplete: function(){
                if(this.check_logged_in())
                {
                    ...
                    this.send_register_request(this.form, (response)=>{
                        ...
                        {
                            var urlsArr = [];
                            $.each(response.redirect, function(k, url){
                             ...
                             var cvalue = JSON.stringify(urlsArr);
            ...
            send_register_request: function(form_values, callback){
                axios.post('/register_sale_test', form_values).then((response)=>{
                    callback(response.data);
                    // $.each(response.data.redirect, function(k, v){
                    //     console.log(v);
                    //     callback(v);
                    // });
                });
            ...
            onSelectPackage: function(price, packageType){
                ...
                this.setPackageCookie('__PT',this.form.packageType,1);
                this.setPackageCookie('__PR',this.form.price,1);
                this.$refs.formWizard.nextTab();
            },
            finishSteps: function(){
                if(this.$refs.personalInfoStep.validateFields()){
                 alert("Per favore, compilare almeno un campo obbligatorio fra quelli presenti evidenziati in rosso.");
                 return false;
                }  
            ...
            onFirstRegisterSaleFormFinished:function(data){
                this.form.firstregistersaleform_data = data;
                this.showFirstRegisterSaleForm = true;
                this.showQuestionnaire = true;
                console.log(this.form.firstregistersaleform_data);
            },
            setPackageCookie:function(cname, cvalue, exdays) {
            var d = new Date();
